@extends('layouts.app', ['activePage' => 'sub_categories', 'titlePage' => __('Sub Categories')])
        ...
        <div class="col-md-12">
                <div class="card ">
                    <div class="card-header card-header-primary">
                        <h4 class="card-title float-left">{{ __('Sub Categories') }}</h4>
                        <a href="{{ route('sub_categories.create') }}" class="btn btn-success float-right"><i class="material-icons">add</i></a>
                    </div>
                    <div class="card-body ">

                        <ul class="accordion" id="sortable">
                            @foreach ($sub_categories as $category_type )
                            <li class="card">
                                <div class="card-header" id="heading{{ $category_type->id }}">
                                    ...
                                    <div class="pull-right">
                                        <form onsubmit="Javascript: return confirm('Are You Sure, Want To Remove This ?');"  method="POST" action="{{ route('sub_categories.destroy', $category_type->id) }}">
                                            @csrf
                                            @method('DELETE')
                                            <a href="{{ route('sub_categories.edit', $category_type->id) }}" class="btn btn-sm btn-success" data-original-title title><i class="material-icons">edit</i></a>
...
<script>
    ...
    {
        $.ajax({
            "url" : "/admin/sub_categories/update_sequence",
            "type" : "put",
            "dataType": "json",
